...
| POST   | `/api/v1/auth/refresh`            | Refresh-token grant. Rotates cookies and returns the same identity payload as `/me`. |
| GET    | `/api/v1/auth/me`                 | Reads the access-token cookie and returns `userId` (the `sub` claim), `username` (`preferred_username`), `email` (falling back to the username when the token carries no email) and `roles`. A claim the token does not carry comes back as an empty string. |
